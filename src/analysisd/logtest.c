...
bool w_logtest_check_input(char * input_json, cJSON ** req, OSList * list_msg) {
    ...
    *req = root;
    ...
    if (!(location && cJSON_IsString(location) && (location->valuestring != NULL))) {
    ...
    if (!(log_format && cJSON_IsString(log_format) && (log_format->valuestring != NULL))) {
    ...
    if (!(event && cJSON_IsString(event) && (event->valuestring != NULL))) {
    ...
    if (token && cJSON_IsString(token) && token->valuestring != NULL && strlen(token->valuestring) != W_LOGTEST_TOKEN_LENGH) {
